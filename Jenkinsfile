pipeline {
  ...
  parameters {
    booleanParam(name: 'DO_RELEASE', defaultValue: false, description: '是否发布到测试仓库（勾选后会弹窗输入版本）')
  }
  ...
  stages {
    ...
    stage('Release to TEST (Manual Version)') {
      when { expression { return params.DO_RELEASE } }
      steps {
        script {
          def v = input(
            ...
            ok: '确认发布到测试仓库',
            parameters: [
              string(name: 'RELEASE_VERSION', defaultValue: 'V.1.1', description: '例如：V.1.1 或 V.1.1.1')
            ]
          ).trim()

          if (!(v ==~ /^V\.\d{1,2}\.\d{1,2}(\.\d{1,2})?$/)) {
            error("版本格式不合法：${v}。应为 V.X.X 或 V.X.X.X（X为数字）")
          }
